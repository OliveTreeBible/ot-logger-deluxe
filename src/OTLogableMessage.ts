...
export class OTLogableMessage {
    ...
    toString(): string {
        return this._messageParts.reduce((prev: string, part: IOTMessagePart, c: number, n: IOTMessagePart[]) => {
    ...
    _partToString(part: IOTMessagePart) {
        let text = part.text.length < 1000 ? part.text : part.text.substring(0,1000)
        text = text.replaceAll("\"", "\\\"")
        return `${part.name ? `*${part.name}:* `: ''}${part.code ? '`' : ''}${text}${part.code ? '`' : ''}`
    }
    ...
    static Create(message: string | IOTMessagePart[]) {
        if(typeof message === 'string')
            ...
            return new OTLogableMessage(message)
    }

    static CreateWithErrorPart(message: string , errorPart: any) {
        return new OTLogableMessage([{ text: message },{ name: 'Upstream Error', text: `${errorPart}`}])
    }
}